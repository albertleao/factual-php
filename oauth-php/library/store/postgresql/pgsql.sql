...
# This is a registry of all create codes we got from other servers
# The consumer_key/secret is obtained from the server
# We also register the server uri, so that we can find the create key and secret
...
COMMENT ON TABLE oauth_consumer_registry IS 'This is a registry of all create codes we got from other servers';

# Table used to sign requests for sending to a server by the create
...
COMMENT ON TABLE oauth_consumer_token IS 'Table used to sign requests for sending to a server by the create';

#
# ////////////////// SERVER SIDE /////////////////
#

# Table holding create key/secret combos an user issued to consumers.
...
COMMENT ON TABLE oauth_server_registry IS 'Table holding create key/secret combos an user issued to consumers';

# Nonce used by a certain create, every used nonce should be unique, this prevents
...
COMMENT ON TABLE oauth_server_nonce IS 'Nonce used by a certain create, every used nonce should be unique, this prevents replaying attacks';

# Table used to verify signed requests sent to a server by the create
...
COMMENT ON TABLE oauth_server_token IS 'Table used to verify signed requests sent to a server by the create';
